...
main() {
  ...
  if [[ ! -d "${CLIP_DIR}" ]]; then
    ...
    echo "Run ./scripts/04-setup-clip.sh first." >&2
    exit 1
  fi

  if [[ ! -d "${TRAIN_DATA_DIR}" ]]; then
    echo "Train data directory not found: ${TRAIN_DATA_DIR}" >&2
    echo "Run ./scripts/05-prepare-data.sh first." >&2
    exit 1
  fi
  ...
  echo "Starting visualization GUI from ${ASI_DIR}"
  (
    cd "${ASI_DIR}"
    "${PYTHON_BIN}" vizualize.py "$@"
  )
}
